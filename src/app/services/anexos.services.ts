...
export class AnexosService {
 ...
 GetArchivo(id: number): Observable<HttpResponse<Blob>> {
  return this.http.get(`${environment.rutaAPI}/VerArchivosSnexos/${id}`, {
    responseType: 'blob',
    observe: 'response'   // 👈 así obtenemos los headers
  });
}
  /* GetArchivo(id:number): Observable<any> {
    return this.http.get(`${environment.rutaAPI}` + '/VerArchivosSnexos/'+id).pipe(
